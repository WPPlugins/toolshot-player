...
function html_toolshot_player($url, $image){
    if(empty($url)) return;
...
function shortcode_player_toolshot($args){
    ...
    return html_toolshot_player($args['url'], $args['image']);
}
    ...
    <script>
        ...
        function toolshot_add_shortcode(){
            if(document.getElementById('th_hand_upload_input_url').value=='') alert('Error url video not empty');
                ...
                tinyMCE.activeEditor.selection.setContent('[toolshot_player image="'+document.getElementById('th_hand_upload_input_image').value+'" url="'+document.getElementById('th_hand_upload_input_url').value+'"]');
