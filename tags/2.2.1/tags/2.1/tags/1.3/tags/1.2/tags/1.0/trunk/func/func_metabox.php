...
function metabox_toolshot_show($content){
    if (is_single()){
        global $post, $url_toolshot_player, $toolshot_config;
        $url = get_post_meta($post->ID, '_th_player_url', true);
		if(empty($url)) return $content;
		
        $url_video = 'p_key='.$toolshot_config['player']->tc_key.'&url='.urlencode(base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($toolshot_config['player']->tc_key), $url, MCRYPT_MODE_CBC, md5(md5($toolshot_config['player']->tc_key)))));
...
?>
    ...
    <ul class="th_tab_ clearfix">
        ...
        <li><a href="#" data-upload="auto_upload" onclick="return th_change_tab(this, 'auto_upload');">Auto Upload</a></li>
    </ul>
    ...
    <script>
        ...
        // get source support
        var source_player = [<?php foreach (json_decode($toolshot_config['player']->tc_option)->source_player as $val) echo '"'.$val.'",';?>];
        ...
        function th_get_video(thiss, val){
            if(typeof val != 'undefined' && th_url_video != val){
                ...
                http.onreadystatechange = function(){
                    ...
                    document.getElementsByClassName('embed-responsive')[0].innerHTML = '<video id="my-video_html5_api" autoplay controls class="embed-responsive-item"><source src="'+obj[0]['file'].replace(/\"/gim, '&#34;')+'" type="video/mp4"></video>';
                };
                http.send(null);
            }
        }
