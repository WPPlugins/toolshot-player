...
<div class="wrap nosubsub">
    <h1>ToolsHot.Com Player Settings</h1>
    ...
    <div id="col-container" class="wp-clearfix">
        ...
        <div id="col-left" style="width:33%;">
            <div class="embed-responsive embed-responsive-16by9">
                <?php
                $url_video = 'p_key='.$toolshot_config['player']->tc_key.'&url='.urlencode(base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($toolshot_config['player']->tc_key), 'https://www.youtube.com/watch?v=iNJdPyoqt8U', MCRYPT_MODE_CBC, md5(md5($toolshot_config['player']->tc_key)))));
                if($tc_text->player == 'toolshot'){
                    $tmp = '';
                    foreach($tc_text as $key => $val) $tmp .= $key.'='.$val.'&'; ?>
                ...
                <?php }else{?>
                    ...
                    <script type="text/javascript">
                        var tc_text = JSON.parse('<?=$toolshot_config['player']->tc_text?>');
                    ...
                    <form method="post" action="#" class="validate view_player_settings_">
                        <div class="form-field form-required term-name-wrap">
                            <div style="float: left;margin-right:20px;">
                                ...
                                <select name="player" id="player">
                                    <option value="toolshot" <?=$tc_text->player=='toolshot'?'selected':''?>>Toolshot (free)</option>
                                    <option value="self_host" <?=$tc_text->player=='self_host'?'selected':''?>>Self Host (vip)</option>
                                </select>
                            </div>
                            <div style="float: left;margin-right:20px;">
                                <label for="skin">Skin</label>
                                <select name="skin" id="skin">
                                    <option value="default" <?=$tc_text->skin=='default'?'selected':''?>>default</option>
                                    <option value="five" <?=$tc_text->skin=='five'?'selected':''?>>five</option>
                                    <option value="glow" <?=$tc_text->skin=='glow'?'selected':''?>>glow</option>
                                    <option value="roundster" <?=$tc_text->skin=='roundster'?'selected':''?>>roundster</option>
                                    <option value="six" <?=$tc_text->skin=='six'?'selected':''?>>six</option>
                                    <option value="stormtrooper" <?=$tc_text->skin=='stormtrooper'?'selected':''?>>stormtrooper</option>
                                    <option value="vapor" <?=$tc_text->skin=='vapor'?'selected':''?>>vapor</option>
                                </select>
                            </div>
                            <label>&nbsp;</label>
                            <div class="checkbox_">
                                <label class="selectit"><input value="1" type="checkbox" name="autoplay" id="in-category-1" <?=$tc_text->autoplay=='true'?'checked':''?>> Autoplay</label>
                                <label class="selectit"><input value="1" type="checkbox" name="download" id="in-category-1" <?=$tc_text->download=='true'?'checked':''?>> Download Button</label>
                                <label class="selectit"><input value="1" type="checkbox" name="rewind" id="in-category-1" <?=$tc_text->rewind=='true'?'checked':''?>> Rewind Button</label>
                                <label class="selectit"><input value="1" type="checkbox" name="fast_forward" id="in-category-1" <?=$tc_text->fast_forward=='true'?'checked':''?>> Fast Forward Button</label>
                            </div>
                        ...
                        <div class="two_col_">
                            <div class="form-field form-required term-name-wrap">
                                ...
                                <input name="logo" id="logo" type="text" value="<?=$tc_text->logo?>" size="40" aria-required="true" placeholder="Url Logo Image or Text">
                            </div>
                            <div class="form-field form-required term-name-wrap">
                                <label for="logo_url">Logo Url</label>
                                <input name="logo_url" id="logo_url" type="text" value="<?=$tc_text->logo_url?>" size="40" aria-required="true" placeholder="Logo Url (Example : http://example.com)">
                            </div>
                        </div><!--.two_col_-->
                        <div class="clear_"></div>
                        <div class="form-field form-required term-name-wrap">
                        ...
                        <div class="two_col_">
                            <div class="form-field form-required term-name-wrap">
                                ...
                                <div class="checkbox_">
                                    <label class="selectit"><input value="topleft" type="radio" name="logo_position" id="logo_position" <?=$tc_text->logo_position=='topleft'?'checked':''?>> Top Left</label>
                                    <label class="selectit"><input value="topright" type="radio" name="logo_position" id="logo_position" <?=$tc_text->logo_position=='topright'?'checked':''?>> Top Right</label>
                                    <label class="selectit"><input value="bottomleft" type="radio" name="logo_position" id="logo_position" <?=$tc_text->logo_position=='bottomleft'?'checked':''?>> Bottom Left</label>
                                    <label class="selectit"><input value="bottomright" type="radio" name="logo_position" id="logo_position" <?=$tc_text->logo_position=='bottomright'?'checked':''?>> Bottom Right</label>
                                </div>
                            </div>
                            <div class="form-field form-required term-name-wrap">
                                <div class="two_col_">
                        ...
                        <div class="ads_ two_col_">
                            <div class="form-field form-required term-name-wrap">
                                ...
                                <div class="checkbox_">
                                    <label class="selectit"><input value="banner" type="radio" name="ads_type" id="ads_type" <?=!isset($tc_text->ads_type)||isset($tc_text->ads_type) && $tc_text->ads_type=='banner'?'checked':''?>> Banner</label>
                                    <label class="selectit"><input value="banner_close_and_play" type="radio" name="ads_type" id="ads_type" <?=isset($tc_text->ads_type) && $tc_text->ads_type=='banner_close_and_play'?'checked':''?>> Banner Close And Play</label>
                        ...
                        <div class="ads_ form-field term-description-wrap">
                            ...
                            <textarea name="ads_code" id="ads_code" rows="4" cols="40"><?=isset($tc_text->ads_code)?$tc_text->ads_code:''?></textarea>
                        </div>
                        <div class="two_col_">
                            <div class="form-field form-required term-name-wrap">
...
<script>
    ...
    slider_($('form.view_player_settings_ *[name=ads_width]'), <?=isset($tc_text->ads_width)?preg_replace('#\D+#mis', '', $tc_text->ads_width):'300'?>, 1000);
    slider_($('form.view_player_settings_ *[name=ads_height]'), <?=isset($tc_text->ads_height)?preg_replace('#\D+#mis', '', $tc_text->ads_height):'120'?>, 1000);
    slider_($('form.view_player_settings_ *[name=ads_x]'), <?=isset($tc_text->ads_x)?preg_replace('#\D+#mis', '', $tc_text->ads_x):'0'?>, 500);
    slider_($('form.view_player_settings_ *[name=ads_y]'), <?=isset($tc_text->ads_y)?preg_replace('#\D+#mis', '', $tc_text->ads_y):'60'?>, 500);

    slider_($('form.view_player_settings_ *[name=logo_size]'), <?=preg_replace('#\D+#mis', '', $tc_text->logo_size)?>);
    slider_($('form.view_player_settings_ *[name=logo_x]'), <?=preg_replace('#\D+#mis', '', $tc_text->logo_x)?>);
    slider_($('form.view_player_settings_ *[name=logo_y]'), <?=preg_replace('#\D+#mis', '', $tc_text->logo_y)?>);
    // form submit
    ...
    $.get('<?=$url_toolshot?>api/get_license', function(data){
        ...
        if(data['p_type']=='Not set'){
            $('.license_').append('<p><b>Type : </b>'+data['p_type']+'</p><p><a href="<?=$url_toolshot?>" target="_blank" style="text-decoration:none;">Buy VIP</a></p>');
        }else{
            if(data['p_type']=='Vip')
                $('.license_').append('<p><b>Type : </b>'+data['p_type']+'</p><p><b>Date Exp (yyyy/mm/dd) : </b>'+data['p_date_exp']+'</p><p><a href="<?=$url_toolshot?>" target="_blank" style="text-decoration:none;">Buy more VIP</a></p>');
            else
                $('.license_').append('<p><b>Type : </b>'+data['p_type']+'</p><p><a href="<?=$url_toolshot?>" target="_blank" style="text-decoration:none;">Buy VIP</a></p>');
        }
    });
