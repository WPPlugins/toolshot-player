...
function html_toolshot_player($url, $image, $player=null){
    if(empty($url)) return;
    ...
    if($toolshot_player['player'] == 'toolshot' || $player!=null && $player=='toolshot'){
        $tmp = 'image='.$image.'&';
        foreach($toolshot_player as $key => $val) if($key=='ads_code' || $key=='source_player' || $key=='search_upload' || $key == 'image'){}else $tmp .= $key.'='.$val.'&';
...
function metabox_toolshot_show($content){
    ...
    $html = '
        ...
        var toolshot_player = {';
        foreach($toolshot_player as $key => $val) if($key=='ads_code' || $key=='source_player' || $key=='search_upload'){}else $html .= $key.' : \''.$val.'\',';
    $html .= '};
...
function shortcode_player_toolshot($args){
    ...
    return html_toolshot_player($args['url'], $args['image'], $args['player']);
}
...
?>
    <style>
        ...
        #th_hand_upload_source p, #th_auto_upload_checkbox p, #th_multi_upload_checkbox p, #th_auto_upload_category p{margin: 0.6em 0;}
        #th_hand_upload_source b, #th_auto_upload_checkbox b, #th_multi_upload_checkbox b, #th_auto_upload_category b{text-transform: capitalize;}
        #th_auto_upload_checkbox label, #th_multi_upload_checkbox label, #th_auto_upload_category label{
            margin-left: 1.1em;
            line-height: 2em;
        ...
        #th_auto_upload, #th_multi_upload{display: none;}
        ...
        .two_col_ .dashicons, .toolshot_add_shortcode_ .dashicons{line-height: 28px;}
        ...
        #th_auto_upload_list_posts .item_, #th_multi_upload_list_posts .item_{
            width:16.66%;
            float:left;
        }
        #th_auto_upload_list_posts .item_ a, #th_multi_upload_list_posts .item_ a{
            text-decoration: none;
            color:rgba(68, 68, 68, 0.78);
        }
        #th_auto_upload_list_posts .item_ a:hover, #th_multi_upload_list_posts .item_ a:hover{
            color:#000;
        }
        #th_auto_upload_list_posts .item_ > div, #th_multi_upload_list_posts .item_ > div{
            padding:0 5px;
            position: relative;
        }
        .quick_upload_edit_ {
        ...
        #th_auto_upload_list_posts .item_ div:hover .quick_upload_edit_, #th_multi_upload_list_posts .item_ div:hover .quick_upload_edit_{
            display: block;
        }
        ...
        #th_auto_upload_list_posts .item_ a div.img_, #th_multi_upload_list_posts .item_ a div.img_{
            height:150px;
            overflow: hidden;
        }
        #th_auto_upload_list_posts .item_ a div.img_ img, #th_multi_upload_list_posts .item_ a div.img_ img{
            width:100%;
        }
        #th_auto_upload_list_posts .item_ p, #th_multi_upload_list_posts .item_ p{
            margin-top: .3em;
            text-align: center;
            height: 3em;
            overflow: hidden;
    ...
    <div id="th_multi_upload">
        ...
        <hr>
        <div id="th_multi_upload_list_posts" class="clearfix">
            <div></div>
        ...
        <div class="two_col_ clearfix">
            ...
            <div class="item_" style="text-align: left;">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" width="640" height="360" src="https://www.youtube.com/embed/dgEFu-aw-Mc" frameborder="0" allowfullscreen></iframe>
                </div>
            </div>
        </div><!--.two_col_-->
    </div><!--#th_about-->
    <script>
        ...
        function quick_upload_edit(obj){
            if(obj['method']=='edit'){
                ...
                tmp_category = '';
                for (i = 0; i < arr_category.length; i++) tmp_category += '&category[]='+arr_category[i];
                params = 'task=upload_post&title='+obj['title']+'&img='+obj['img']+'&url='+obj['url']+tmp_category;
                http.open("GET", 'admin.php?page=toolshot_player_view_player_settings&'+params, true);
                http.onreadystatechange = function(){};
                http.send(null);
        ...
        function upload_all(thiss, element_id){
            ...
            for(i=0; i < arr_tmp.length; i++)
                if(tmp.length<1745){
                    tmp += '&title[]='+arr_tmp[i]['title']+'&img[]='+arr_tmp[i]['img']+'&url[]='+arr_tmp[i]['url'];
